"""Score the frozen FUT-011 HYPE 120-minute loss-close gross pilot.

Requires DuckDB. Only the two SHA-pinned 2025 Parquet shards in FUT-011 are
...
SOURCES = {
    ...
    "2025-09-28": "c858ffcfca963b858ed3d696f569b38178bbfec1fdb897adca8cd7a6ccc75a0a",
}
SEGMENTS = (
    ("2025-08-31", "2025-08-31T03:00:00", "2025-08-31T10:00:00"),
    ("2025-09-28", "2025-09-28T03:00:00", "2025-09-28T10:00:00"),
)
COINS = ("HYPE",)
FIVE_MIN_MS = 5 * 60_000
MINUTE_MS = 60_000
HOLD_MS = 120 * MINUTE_MS
...
def score(august: Path, september: Path) -> dict[str, object]:
    segments = segment_ms()
    ...
    source_counts = {
        "2025-08-31": load_source(august, "2025-08-31", segments, flow, trades),
        "2025-09-28": load_source(september, "2025-09-28", segments, flow, trades),
    }
    ...
    for date, start, end in segments:
        t = start
        while t + FIVE_MIN_MS + MINUTE_MS + HOLD_MS + PROXY_WINDOW_MS <= end:
            for coin in COINS:
    ...
    sample_pass = all(by_date[date]["n"] >= 50 for date in SOURCES) and total["n"] >= 100
...
def main() -> None:
    ...
    parser.add_argument("august_parquet", type=Path)
    parser.add_argument("september_parquet", type=Path)
    args = parser.parse_args()
    print(json.dumps(score(args.august_parquet, args.september_parquet), indent=2, sort_keys=True))
