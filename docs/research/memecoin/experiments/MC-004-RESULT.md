...
A later account-owner check using the pinned event's `user` field matched the owner of the event-sized token account in **47/48** single-event transactions. The exception was a routed sell: the emitted user was an intermediary, while the original token source had a different owner. The reproducible [user-owner audit](../data/mc004-user-owner-audit-20260924.json), SHA-256 `c4e90e4ae77ac620d2a02c82922f6457041ac178e1b349c264d51093c8fd9274`, preserves all 48 signatures and response hashes. Run `memecoin_audit.py` on an extracted MC-003 archive with current code, then `memecoin_user_owner_audit.py MC003_DIRECTORY MC004_DIRECTORY --out OUTPUT`. Even a matching account owner does not by itself establish independent beneficial control or a wallet-funded buy.
