...
A later diagnostic of the twelve last-observed exit states found real SOL curve reserves of **1–18 lamports in eleven** and **2,074,072 lamports in one**. Most had returned to the initial virtual reserve configuration after selling. This explains why the frozen curve sell quote could not support the stressed inventory. It does not establish that every alternate venue was unavailable or that zero recovery was realized; the `EXIT_UNAVAILABLE` label is specific to the modeled Pump curve route and observation time.
