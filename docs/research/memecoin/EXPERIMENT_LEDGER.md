...
Parent MC-005. Status `FROZEN` before a new capture. [Design](experiments/MC-006-FROZEN.md): 600-second bounded prospective cohort with a batch-synced collector, independent signature reconciliation, unchanged flow score and quote/risk policy, exact uncertainty on top-versus-rest unavailable exits, minute-level CreateEvent receipt age, and event-user uniqueness as a diagnostic only. No result is claimed before its coverage and protocol gate.
