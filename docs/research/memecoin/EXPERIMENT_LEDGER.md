...
Parent MC-003/MC-004. Status `CANDIDATE` for the exit-risk association, `REJECTED` as an entry rule. [Frozen design](experiments/MC-005-FROZEN.md) and [result](experiments/MC-005-RESULT.md). A fresh 600-second cohort matched 152,120/152,120 second-provider interior signatures; 141 launches were scored. The top flow quartile had 1/25 development and 0/11 evaluation unavailable curve exits, versus 19/57 and 14/31 among other entry-known names. All predeclared held-out minimum gates passed, but development stress returns were negative and one +288.65% modeled winner drove the positive held-out mean; leave-one-out held-out top means were negative under both fee scenarios. Failure attribution: `COST_DRAG`, `DATA_LIMITATION`/outlier sensitivity, `EXECUTION_UNREPRODUCIBLE` for hypothetical orders, and episodic receipt latency. Allowed: independent risk replication. Forbidden: an accepted `ENTER` rule or economic edge claim. Archive and offline replay preserve the trial; implementation SHA pending final commit.
