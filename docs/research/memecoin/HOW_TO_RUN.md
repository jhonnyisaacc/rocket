...
MC-011 uses that extracted MC-010 directory as source. Its frozen selection is `data/mc011-selection-20260924.json` from commit `aac0693`. Run `python scripts/research/memecoin_early_actor_audit.py prepare MC010_DIRECTORY --out NEW_SELECTION.json` and compare it to the committed selection. The evidence archive `data/mc011-actor-audit-20260924.tar.gz` (SHA-256 `edd82a6033411784383b223913076fc046c1dd18fede758bc07d76f9cffd6c06`) contains all 144 saved RPC responses and `report.json`; after extraction, run `python scripts/research/memecoin_early_actor_audit.py report MC010_DIRECTORY SELECTION.json --responses RESPONSES_DIRECTORY --out REPLAY.json` and compare to the archived report. The later `getTransaction` clocks remain later truth checks. Do not use this already evaluated panel as a new strategy holdout.
