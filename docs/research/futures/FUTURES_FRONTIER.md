...
Next decision: reconstruct scheduled delisting and forced-settlement outcomes from dated official notices and checksum-verified minute price-index/trade archives. Distinguish a failed next-open entry from a position automatically settled before its scheduled exit. Rerun the causal preflight and then the frozen `FUT-001` cheap gate. The acquired tape is broad enough for the question; treating the remaining exposed-day gaps as zero would not be defensible. Do not move the declared OOS window after seeing outcomes.
