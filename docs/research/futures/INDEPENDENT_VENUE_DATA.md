...
The [minute settlement probe](../../../research/futures/bybit_settlement_probe.py) retrieved and hashed both Bybit trade and index 1-minute responses around the three metadata delivery times. All 30- and 60-minute pre-cutoff index windows are complete, and each last active trade minute immediately precedes its cutoff. The [Bybit API announcement](https://t.me/s/Bybit_API_Announcements?before=94) reports **LUNA settled at 0.024 USDT at 10:03 UTC May 12**, which matches the index candle at that minute. Its pre-cutoff 30-minute index-close average is about 0.1541, so a generic average rule would materially misstate this special closure. FTT and SRM have index-envelope candidates but not independently verified exact settlement prices. Ignored local minute-probe SHA-256: `08716ca14f67c3990de898d0e8841aa027fef3e922a8435d9fb404bceae1230c`.
...
The existing baseline's [FUT-003 Bybit venue-transfer diagnostic](experiments/FUT-003.md) is frozen before Bybit outcomes; its cost and FTT/SRM settlement bounds are explicitly provisional. A **distinct** future gross-signal hypothesis still needs a separate frozen contract and independent outcome. No 2026 Binance final-holdout bars should be inspected for candidate selection.
